...
function ForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEmail) {
            try {
                const response = await axios.post("http://localhost:3000/api/email/forgotpassword", { email: email });
                setSuccess(true);
            ...
            // console.log(mobile);
            try {
                const response = await axios.post("http://localhost:3000/api/mobile/forgotPassword", { mobile: mobile });
                setSuccess(true);
                setMessage(response.data.message);
                console.log(response.data.message);
            }
